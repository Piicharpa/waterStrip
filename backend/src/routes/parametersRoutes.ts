...
router.post("/", async (req, res) => {
  ...
  const [newParameter] = await dbClient.insert(Parameter).values({ p_name, p_unit , p_min , p_max }).returning();
...
router.patch("/:id", async (req, res) => {
  ...
  const [updatedParameter] = await dbClient.update(Parameter).set({ p_name, p_unit , p_min , p_max }).where(eq(Parameter.p_id, Number(id))).returning();
